...
example_max_sust <- example_winds[["vmax_sust"]]
example_surface_dir <- example_winds[["surface_wind_direction"]]
...
practice <- example_plot[example_plot$rowname %in% test$gridid,]
 ...
 ggplot()+
  ...
  geom_spoke(data = practice, aes(x = glon, y = glat,
               ...
               show.legend = TRUE)+
  scale_color_gradient(low='darkblue', high='red')+
  ggtitle("Max Sustained Winds and Direction at", paste0(example_time))
  #geom_point(data = storm_center, aes(x = tclon, y = tclat), color = "green")
