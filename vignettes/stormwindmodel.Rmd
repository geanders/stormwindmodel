...
2. To get the old (`mda2`) angle and the new (`mda`) angle to be similar, I need to add 90 degrees from the angle calculated by the bearing equation (this is the "+ 90" in the equation above. I think this might be because we currently are using longitudes that are expresses as positive values, even though we're in the Western hemisphere.
3. Any reason to use the old method versus the new one? They give very similar results (see below)
