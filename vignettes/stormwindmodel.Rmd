...
- From talking with John and Andrea, we might want to bias-correct this model based on extended tracks wind radii from 2004 on (when we have pretty complete wind radii values in the extended best tracks). Although, from talking more with John, the extended tracks wind radii give the *maximum* wind radius in each quadrant, so this means that there will be a lot of false positives (many counties that are in a quadrant, but aren't at the angle where the maximum radius in that quadrant occurs). Good storms to check would be Katrina, Ike, Charley, and Ivan. We should make sure to compare results from our model to extended tracks wind radii *before* adjusting by the wind gust factor, so we're comparing measures of sustained surface wind speed from both sources (note: I now have the last function output both sustained and gust wind speeds, so we have the sustained wind speeds that we can compare to the extended hurricane tracks wind radii).
...
Here are some values from doing this process for the county center of Dare County, NC, during Hurricane Floyd (`track` is the gradient rotational wind speed, in meters per second):
...
a <- ggplot(grid_wind, aes(x = track)) + geom_histogram() + 
  theme_tufte()
b <- ggplot(grid_wind, aes(x = -lon, y = phi, color = track)) + 
  geom_point() + 
  geom_point(data = grid_point, aes(x = glon, y = glat), color = "red") + 
  theme_tufte() + 
    scale_colour_gradientn(colours=rainbow(4))
...
The graph on the right shows the wind speed in Dare County, NC, for each time on the storm's track. In other words, Dare did not have storm-related winds when the storm was just starting, but had winds over 20 m / s when the storm was closest.
...
### Calculate the angle between the storm center and each location

So far, the model has just calculated the the rotational component of the wind speed at each grid location (tangential wind). Now it needs to add back in the forward motion component of the wind speed (translational wind) at each location. This requires, for each grid point, adding the separate vectors of tangential and translational winds. When these vectors point in the same direction, the total storm winds will be increased by the forward motion (the right side of the storm). Conversely, on the left side of the storm, the forward motion of the storm will offset some of the storm-related wind. 
...
First, we need to determine, for each storm location, the direction from the storm center to the locations. For this, we can use the `calc_bearing` function that we used earlier to determine the bearing of the storm at each storm observation. Now, we use it to calculate the angle from the storm center to the grid point, and then add 90 degrees to get the angle of the tangential winds:
...
Here is a plot of the bearing of Dare County, NC, from the storm at each storm observation for Hurricane Floyd:
...
grid_wind <- mutate(with_wind_radii,
                      ...
                      # grid point
                      bearing_from_storm = stormwindmodel:::calc_bearing(phi,
                                                                        lon,
                                                              grid_point$glat,
                                                          - grid_point$glon),
...
ggplot(grid_wind, aes(x = oldangle, y = bearing_from_storm)) + geom_point() +
  theme_tufte()
coef(lm(gwd ~ gwd2, data = grid_wind))
```

The old and new methods seem to agree very well. Here are the bearings from the storm for this sample county over time, so we can check if the directions are reasonable. 

```{r echo = FALSE, fig.width = 5, fig.height = 3}
ggplot(grid_wind, aes(x = -lon, y = phi, color = bearing_from_storm)) +
  geom_point() + 
  geom_point(data = grid_point, aes(x = glon, y = glat), color = "red") +
  theme_tufte() + 
    scale_colour_gradientn(colours=rainbow(4)) 
...
The direction seems to change from positive to negative right when the storm passes, which seems right since the bearing is changing from northeast to southeast, and due east is 0 degrees. I think that could also be a good sign that this model is working reasonably well for getting bearing from the storm. 
Next, we calculate the gradient wind direction based on the bearing of a location from the storm. This should be the bearing from the storm plus 90 degrees. Here is a map of that for the same county: 

```{r echo = FALSE, fig.width = 5, fig.height = 3}
ggplot(grid_wind, aes(x = -lon, y = phi, color = gwd)) +
  geom_point() + 
  geom_point(data = grid_point, aes(x = glon, y = glat), color = "red") +
  theme_tufte() + 
    scale_colour_gradientn(colours=rainbow(4)) 
...
This also looks right. Winds are due east (0 degrees) when the county is directly south of the storm.
...
The old code adds 20 degrees for this inflow angle. However, I think there are two ways we can improve this. First, in looking over some papers, I found the following equation for inflow angle as a function of the distance from the storm center to the location and the storm's $R_{max}$ at that observation point (Phadke et al., 2003):
...
I have changed the code to use this equation rather than always using an inflow angle of 20 degress.

Second, in talking with John and Andrea, it sounds like overland the inflow angle should be about 20 degrees more than it is over water. Therefore, after calculating the inflow angle from the equation above, I then am adding 20 degrees to the value since all of our locations are over land. The final calculation, then, is:
...
Here is the old code for this step:
...
- From talking with John and Andrea, we need to check into whether this calculation should be moved after the part where we add the asymmetry. I think we might be able to figure out the right order by looking at the NOAA 23 document. 

### Go from gradient level to surface level winds

At this point in the old code, there was some code that did two things at one: 
1. Broke the surface wind speed down into u- and v-components and;
2. Applied the factor of 0.9 to bring wind speeds back from gradient level to surface level (very early in the code, we use 1 / 0.9 to change from surface level wind speed in best tracks to gradient level wind speed to generate the wind profile). 

I don't think we need to break the surface wind into u- and v-components anymore. I imagine that sometime a while ago, there was a spot in the code where the u- and v-components of forward wind speed were being added in directly before the u- and v-components of total wind speed were added back together to get the total magnitude of the wind speed. However, now it seems we're using an asymmetry adjustment to add back in the forward wind speed, and the old code seemed to add the u- and v-components back together immediately without adding forward speed components to them, so I've taken this part of the code out of the new version. I do still, however, apply the 0.9 factor at this point to bring winds back down from gradient to surface level:
...
- From John and Andrea: This looks like it's just reversing what we did before, when we went from surface to gradient, so we could remove friction when calculating our estimates. However, it looks like this step does *not* adjust for the added friction over land. It sounds like at this point we probably want to do some adjustment for wind speed that accounts for all the grid points being land-based ones. So, I've added that into the code after we apply the asymmetry factor. 

### Apply an asymmetry factor

Next, to add back in the storm's forward motion at each grid point, it looks like we are adding in an asymmetry factor that incorporates both the magnitude and the direction (relative to surface winds at a given location) of the storm's forward motion. This equation looks a little funny because there is a constant ($0.514791^{0.37}$) to adjust for the fact that our wind speeds are in meters per second rather than knots:
...
The same report describes $\beta$ as "the angle between the track direction ($\theta$) and the surface wind direction." (Note: Since this specifies "surface wind direction" rather than "gradient wind direction", I think that we add the inflow angle before calculating this asymmetry factor.)
...
### Apply a correction for overland versus overwater

From one source, here are onshore-to-overwater wind ratios that we might want to consider for adjusting from an over-weater estimate to an over-land estimate (from NOAA 23, p. 31, for 10-meter, 10-minute wind speed, to be added after adjusting for asymmetry it looks like): 
...
There was not anything to do this in the old code. In the new code, I have added in the following code to adjust for the fact that we're trying to estimate winds over land, rather than over water, using the "water-to-land" ratio given in the table above: 
...
- I wonder if we could improve the model by adding a county-specific frictional factor. Based on the NOAA 23 report, the 0.89 value is really best only fairly close to the coast, and would not work well for places, in particular, in the mountains.
...
$$
windspd = \mbox{gust factor}*windspd 
$$
