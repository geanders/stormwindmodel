...
1. Impute location and maximum wind speed to 15-minute intervals
2. For each storm track location, calculate all the inputs needed for the Willoughby windspeed model (forward speed, direction of forward motion to the storm, gradient-level wind speed, radius of maximum wind speed, parameters for decay of winds away from the storm's center). 
3. For each county center (or other grid point), estimate wind at all county centers. This step includes: measuring distance to county from storm center; calculating tangential wind components at county; calculating gradient wind direction at each county; determining direction at each county; calculating surface wind direction and backward wind direction.

Note:

- From talking with John and Andrea, we might want to bias correct this model based on extended tracks wind radii from 2004 on (when we have pretty complete wind radii values in the extended best tracks). Good storms to check would be Katrina, Ike, Charley, and Ivan. Make sure you compare before adding the wind gust factor, so you're comparing similar values.
...
(Based on talking with John and Andrea, it probably makes more sense for us to interpolate between 6-hourly observation points by using *splines* rather than lines.)
...
ggplot(with_wind_radii_2, aes(x = cspeed, y = forward_speed)) + geom_point() + 
  geom_abline(intercept = 0, slope = 1)
cor(with_wind_radii_2$cspeed, with_wind_radii_2$forward_speed, use = "complete.obs")
coef(lm(forward_speed ~ cspeed, data = with_wind_radii_2))
```

As you can see, the two are almost perfectly correlated. Further, with the changes made to the `cspeed` function, the values generated when calculating forward speed using the new `forward_speed`) versus old (`cspeed`) methods are nearly identical.
...
The extended tracks database gives maximum winds as one minute sustained wind speeds 10 meters above the ground.
...
- Don't change this yet (because it might be that we should change this the same way now when we go from surface to gradient and later when we transfer back down to surface). However, we might want a different value here depending on whether the storm is over land or over water. One was to do this would be to see how close the storm is to its closest county. If it's not very, very close, it's probably over water. If we want to do this over land, it should be about 20% lower than over water (so, if the value for over water is 0.9, over land it should be 0.9*0.9 = ~0.7). 
...
- From talking with John and Andrea, this could be the place where we could most-improve out models. There are a lot of equations for calculating the radius of maximum windspeed, and there are also estimates based more on radius data from the extended tracks database (and this is probably based more on observations for the real storm). We could potentially try out different equations for this part to optimize our model.
...
This can also be called the "tangential wind".
