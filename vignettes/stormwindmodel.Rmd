...
The old code adds 20 degrees for this inflow angle. However, in talking with John and Andrea, it sounds like an inflow angle of 40 degrees might be more reasonable over land (which is where all the locations we care about are), so I've changed the code to use a 40 degree inflow angle.
...
- Do we want to do something more complex with this than always adding 40 degrees? I found an equation based on the relationship between the distance from the storm center and the storm's $R_{max}$ at that observation point (Phadke et al., 2003). Do we want to use that instead?
...
- From talking with John and Andrea, we probably also want to put this calculation after the part where we bring gradient values back down to surface estimates (although, now that I think about it more, I'm thinking since that just changes wind speed and this just calculates angle, maybe the order doesn't matter).
...
Now the model must add together the two vectors of wind speeds for the grid point, the vector of forward storm motion $\left(\vec{F}\right)$ and the vector of the storm wind $\left(\vec{S}\right)$. To add the two vectors together, we need to break them each into north-south component $\left(v(\vec{F})\right)$ and east-west component $\left(u(\vec{F})\right)$, where a positive value of $v$ indicates northward motion (or wind from the south) and a positive value of $u$ indicates eastward motion (or wind from the west).
...
From one source, here are onshore-to-overwater wind ratios that we might want to consider for adjusting from an over-weater estimate to an over-land estimate (from NOAA 23, p. 31, for 10-meter, 10-minute wind speed): 
...
- After meeting with John and Andrea, I'm pretty sure we're not making any of the corrections in this table. I think that the 0.9 that we apply in this step is to get from gradient to surface winds (the reverse of what we applied at the start of the code to get from surface to gradient).
...
The last step in the code calculates gust wind speed from sustained wind speed by applying a gust factor of 1.3:

$$
windspd = 1.3*windspd 
$$

In one of the sources I was looking at, there's this table for this gust factor: 
...
- Also based on that conversation, we probably want to update the 1.3. It sounds like this gust factor is likely larger over land (the table I put in from one of the sources suggests a value of near 1.5 for inland locations).
...
write.csv(county_points, file = "../writing/county_points.csv")
```
