...
The main function of this package is `get_grid_winds`. It inputs storm tracks for a tropical cyclone (`hurr_track`) and a dataframe with grid point locations (`grid_df`). It models winds during the tropical storm at each grid point and outputs summaries of wind during the storm at each grid point from the storm. The code assumes that tracking data entered is in knots, at 10 m above surface level, and measured with a 1-minute averaging period. The wind measurements generated for each grid point are: 
...
- Has the following columns: 
    ...
    + `longitude`: A numeric vector with longitude in decimal degrees, in
                  degrees East.
    + `wind`: A numeric vector with maximum storm wind speed in knots.
- Data is at 10 m above surface level and measured with a 1-minute averaging period.

If you choose to use IBTrACS (https://www.ncdc.noaa.gov/ibtracs/) to source track data, it is best to use the column called "USA WIND". This column gives the maximum sustained surface windspeed in knots. This specific column in the data uses 1-minute averaging periods at 10 m above surface level, measurements that are assumed by the code in this package.
