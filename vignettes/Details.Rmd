...
Here is an example of running that functions, and the output you get (notice that it only outputs wind estimates for one grid point at a time; in this example, the point is for Dare County, NC, during Hurricane Floyd):
...
The first step is to determine the distance between the grid point and the center of the storm ($C_{dist}$). As with the previous main function, here I am using the Haversine method of great circle distance to calculate this distance. For this, the package uses the `latlon_to_km` function given in the code shown above.
...
First, if $r \le R_1$ (inside the transition region), you are supposed to use this equation to calculate $V_G(r)$ (Willoughby et al. 2006, Eqn 1 (a)):

$$
V_G(r) = V_i = V_{max,G} \left( \frac{r}{R_{max}} \right)^n, (0 \le r \le R_1)
$$

Next, if $r \ge R_2$ (outside the transition region), you are suppose to use this equation to calculate $V(r)$ using this equation (Willoughby et al. 2006, Eqn 4; dual-exponential replacement of Eqn 1 (b)):
